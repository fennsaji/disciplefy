  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedConfig = {
      ...
      features: {
        ...
        memory_verses: formData.features?.memory_verses ?? 0,
        practice_modes: formData.features?.practice_modes ?? 0,
        practice_limit: formData.features?.practice_limit ?? 0,
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Memory Verses Quota (Use -1 for unlimited)
                </label>
                <input
                  type="number"
                  value={formData.features?.memory_verses ?? 0}
                  onChange={(e) => setFormData({
                    ...formData,
                    features: {
                      ...formData.features,
                      memory_verses: parseInt(e.target.value)
                    }
                  })}
                  className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                  min="-1"
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Practice Attempts per Verse
                </label>
                <input
                  type="number"
                  value={formData.features?.practice_limit ?? 0}
                  onChange={(e) => setFormData({
                    ...formData,
                    features: {
                      ...formData.features,
                      practice_limit: parseInt(e.target.value)
                    }
                  })}
                  className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                  min="-1"
                />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Use -1 for unlimited</p>
              </div>
            </div>
          </div>
