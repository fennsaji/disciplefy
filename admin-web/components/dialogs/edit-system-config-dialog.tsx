...
interface EditSystemConfigDialogProps {
  ...
  config: any
  section: 'token_system' | 'voice_features' | 'maintenance_mode' | 'app_version' | 'trial_config' | 'memory_verses' | 'cost_control'
}
...
export default function EditSystemConfigDialog({
  ...
  const renderCostControlFields = () => (
    <div>
      ...
      <input
        ...
        min={1}
        step={1}
        value={formData.daily_cost_limit_usd ?? 15}
        onChange={(e) => {
          const parsed = parseFloat(e.target.value)
          setFormData({ ...formData, daily_cost_limit_usd: Number.isNaN(parsed) ? '' : parsed })
        }}
        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
      />
      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
        What the app may spend with the model providers in a day. Over it, new studies stop and
        readers are asked to try again tomorrow; learning-path studies keep working from the cache.
        Takes effect within a minute.
      </p>

      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 mt-4">
        Monthly pre-warm budget (USD)
      </label>
      <input
        type="number"
        min={0}
        step={1}
        value={formData.prewarm_monthly_budget_usd ?? 20}
        onChange={(e) => {
          const parsed = parseFloat(e.target.value)
          setFormData({ ...formData, prewarm_monthly_budget_usd: Number.isNaN(parsed) ? '' : parsed })
        }}
        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
      />
      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
        What the scheduled pre-warm may spend a month generating learning-path guides ahead of time,
        at half price on the Batch API. When it is spent the job waits for next month. Set it to 0
        to pause pre-warming.
      </p>
    </div>
  )
  ...
  const getSectionTitle = () => {
    switch (section) {
      ...
      case 'cost_control': return 'Edit Daily Spend Ceiling'
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-md w-full mx-4">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {section === 'cost_control' && renderCostControlFields()}
            ...
            <p className="text-xs text-green-800 dark:text-green-200">
              ✅ <strong>Database Update:</strong> {
                section === 'token_system'
                  ...
                  ? 'The spend ceiling will be saved to system_config table'
