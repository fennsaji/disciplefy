...
export function PlByTierTable({ data, exchangeRateUsed, exchangeRateIsLive }: PlByTierTableProps) {
  return (
    <div className="rounded-lg bg-white shadow-md dark:bg-gray-800 dark:shadow-gray-900">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full">
          <thead className="border-b border-gray-200 dark:border-gray-700">
            <tr>
              <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-sm font-medium text-gray-600 dark:text-gray-400 sticky left-0 z-20 bg-white shadow-[2px_0_5px_rgba(0,0,0,0.06)] dark:bg-gray-800">Plan</th>
              <th className="px-3 py-2 sm:px-6 sm:py-3 text-right text-sm font-medium text-gray-600 dark:text-gray-400">Active Users</th>
              <th className="px-3 py-2 sm:px-6 sm:py-3 text-right text-sm font-medium text-gray-600 dark:text-gray-400">Revenue</th>
              <th className="px-3 py-2 sm:px-6 sm:py-3 text-right text-sm font-medium text-gray-600 dark:text-gray-400">LLM Cost</th>
              <th className="px-3 py-2 sm:px-6 sm:py-3 text-right text-sm font-medium text-gray-600 dark:text-gray-400">Gross Profit</th>
              <th className="px-3 py-2 sm:px-6 sm:py-3 text-right text-sm font-medium text-gray-600 dark:text-gray-400">Margin %</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            {data.map((item) => (
              <tr key={item.plan_code} className={`${rowClass(item)} group`}>
                <td className="px-3 py-2 sm:px-6 sm:py-3 text-sm font-medium text-gray-900 dark:text-gray-100 sticky left-0 z-10 bg-white shadow-[2px_0_5px_rgba(0,0,0,0.06)] group-hover:bg-gray-50 dark:bg-gray-800 dark:group-hover:bg-gray-700">
                  {PLAN_LABELS[item.plan_code] ?? item.plan_code}
                </td>
                <td className="px-3 py-2 sm:px-6 sm:py-3 text-right text-sm text-gray-600 dark:text-gray-400">
                  {(item.active_users ?? 0).toLocaleString()}
                </td>
                <td className="px-3 py-2 sm:px-6 sm:py-3 text-right text-sm text-gray-900 dark:text-gray-100">
                  {money(item.revenue_inr, exchangeRateUsed)}
                </td>
                <td className="px-3 py-2 sm:px-6 sm:py-3 text-right text-sm text-gray-900 dark:text-gray-100">
                  {money(item.llm_cost_inr, exchangeRateUsed)}
                </td>
                <td className={`px-3 py-2 sm:px-6 sm:py-3 text-right text-sm font-medium ${
      ...
      <div className="px-3 py-2 sm:px-6 sm:py-3 text-xs text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-gray-700">
        {exchangeRateIsLive
