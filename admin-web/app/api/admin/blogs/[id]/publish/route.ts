...
export async function POST(
  ...
  try {
    ...
    const rsResponse = await fetch(`${RS_BACKEND_URL}/api/v1/admin/posts/${id}/publish`, {
      method: 'POST',
    ...
    if (!rsResponse.ok) {
      return NextResponse.json({ error: data.message || 'Failed to publish post' }, { status: rsResponse.status })
    }
