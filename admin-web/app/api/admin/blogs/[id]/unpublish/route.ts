...
export async function POST(
  ...
  try {
    ...
    const rsResponse = await fetch(`${RS_BACKEND_URL}/api/v1/admin/posts/${id}/unpublish`, {
      method: 'POST',
    ...
    if (!rsResponse.ok) {
      return NextResponse.json({ error: data.message || 'Failed to unpublish post' }, { status: rsResponse.status })
    }
