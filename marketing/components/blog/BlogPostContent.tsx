    ...
    <>
      ...
      <header className="relative overflow-hidden border-b border-[var(--border)]">
        ...
        <div className="relative max-w-[60rem] mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-12 lg:pr-[240px]">
          ...
          {/* Learning Path badge — brand gold, not indigo: this is the one
              marker on the page that says "you're on a structured path",
              same semantic as progress/milestone marks in the app, and it
              needs to read as different from the plain indigo topic tags
              right above it. Gold-on-gold-wash tops out at ~4.3:1 for normal
              text on the light surface, short of AA, so the icon/wash/border
              carry gold and the label stays in body text color — same split
              used for XP counts in the app. */}
          {post.learning_path && (
            <Link
              href={`/blog?learning_path=${post.learning_path.slug}`}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 mb-5 rounded-lg
                         bg-gold-light/40 dark:bg-gold/10
                         border border-gold/30 dark:border-gold/25
                         text-sm font-medium text-gray-800 dark:text-slate-200
                         hover:border-gold/60 transition-colors"
            >
              <svg aria-hidden="true" className="w-3.5 h-3.5 text-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
