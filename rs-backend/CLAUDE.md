...
| `fellowship_daily_post` | Every minute — posts each fellowship's *current lesson* from its own `fellowship_study` row (not the global topic catalog) once its IST posting time (`daily_post_time`) has passed; mentor-controlled cadence (`daily_post_frequency_days`), auto-advance (`daily_post_auto_advance`, lesson pacing within a path) and, independently, `daily_post_auto_advance_path` (whether it may pick a *new* path on its own once the current one is exhausted — off leaves it for the mentor to assign), skip (`daily_post_skip_date`, counts as a posting day) and pause (`daily_post_paused_until`); a failed generation waits an hour (`daily_post_last_failed_at`); reuses a mentor preview's teaser for the same lesson and date (`discipler_daily_post_previews`); switches to the next active learning path on completion when `daily_post_auto_advance_path` is on; before inserting, calls `fellowship-posts/daily-teaser` (`services::fellowship_teaser`) for an LLM-written hook/body to replace the plain summary lead — any failure (timeout, non-200, bad/oversized fields) falls back to the plain template, never fails the post | `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY` (calls `fellowship-posts/notify`), `INTERNAL_API_KEY` (calls `fellowship-posts/daily-teaser`) |
| `discipler_reply_worker` | Every minute (drains reply queue; releases due quiet-hours pushes via `fellowship-posts/flush-pushes`; flushes hourly activity digests on minute 0) | `INTERNAL_API_KEY`, `SUPABASE_ANON_KEY` (calls `fellowship-posts/discipler-reply` and `fellowship-posts/flush-pushes`), `SUPABASE_SERVICE_ROLE_KEY` (calls `fellowship-posts/notify` for digests) |

- Mentor daily post requests (`discipler_daily_post_requests`: post now, preview, new teaser, post again; each gated by an admin flag on `fellowships`) are not handled by the cron job: `fellowship_daily_post::run_request_worker`, spawned in `main.rs`, polls every 5 seconds and runs up to 5 at once, independent of the schedule and of a long posting run. It still obeys the `fellowship_daily_post` enabled switch.
