-- Fix get_pl_by_tier and get_top_heavy_users to include all current subscription statuses.
-- The previous version only filtered on status = 'active', missing 'trial', 'in_progress', etc.
...
BEGIN
  ...
  FROM user_costs uc
    ...
    JOIN subscriptions s ON s.user_id = uc.user_id
      AND s.status IN ('active', 'trial', 'in_progress', 'pending_cancellation', 'paused')
