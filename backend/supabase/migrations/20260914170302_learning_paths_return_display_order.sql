-- The fellowship "Pick a learning path" sheet showed paths out of order within
-- a level: The Gospel of Mark above New Believer Essentials among seekers.
--
-- The listing is ordered for the reader personally (their in-progress, then
-- enrolled, then featured paths) and only then by display_order, and the
-- function never returned display_order, so the app could not restore the
-- curated order where it matters. It now returns display_order as a final
-- column; the ordering itself is unchanged.
--
-- A new return column means the function has to be dropped and recreated.
-- It is SECURITY DEFINER and accepts any p_user_id, so it is limited to the
-- service role, which is the only caller (the learning-paths Edge Function).

DROP FUNCTION IF EXISTS public.get_available_learning_paths(uuid, character varying, boolean, integer, integer, character varying, character varying);

CREATE FUNCTION public.get_available_learning_paths(p_user_id uuid DEFAULT NULL::uuid, p_language character varying DEFAULT 'en'::character varying, p_include_enrolled boolean DEFAULT true, p_limit integer DEFAULT 10, p_offset integer DEFAULT 0, p_category character varying DEFAULT NULL::character varying, p_search character varying DEFAULT NULL::character varying)
...
REVOKE ALL ON FUNCTION public.get_available_learning_paths(uuid, character varying, boolean, integer, integer, character varying, character varying) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_available_learning_paths(uuid, character varying, boolean, integer, integer, character varying, character varying) TO service_role;
