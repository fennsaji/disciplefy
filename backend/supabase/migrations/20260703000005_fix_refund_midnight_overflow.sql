-- Migration: Fix refund_user_tokens midnight overflow (F34)
-- Problem: If a UTC-midnight daily reset fires between token consumption and refund,
-- available_tokens is already restored to daily_limit by the reset. Adding the refund
-- on top pushes available_tokens above daily_limit.
-- Fix: Cap available_tokens at daily_limit on refund; also don't subtract from
-- total_consumed_today below 0 (already guarded by GREATEST).
...
BEGIN
  ...
  -- Cap available_tokens at daily_limit: if a midnight reset already restored
  -- available_tokens to daily_limit, adding more would push it over the cap.
  UPDATE user_tokens
  SET
    available_tokens = LEAST(user_tokens.available_tokens + p_daily_tokens_used, user_tokens.daily_limit),
