-- ============================================================================
-- MIGRATION: Allow Service Role to Access Token Usage Functions
-- Date: 2026-01-10
-- Description: Modify auth checks in token usage functions to allow service_role
--              to call them on behalf of authenticated users (Edge Functions)
--
-- Issue: Migration 20260110000003 added auth.uid() checks that block service_role
--        Edge Function validates authentication, then calls RPC as service_role
--        auth.uid() returns NULL for service_role, causing "Not authorized" error
--
-- Solution: Check current_user role - allow service_role to bypass auth.uid() check
--           Edge Function already validates user authentication before calling
-- ============================================================================

BEGIN;

-- ============================================================================
-- Fix 1: Update get_user_token_usage_history() to allow service_role
-- ============================================================================
...
COMMENT ON FUNCTION public.get_user_token_usage_history IS
  'Retrieves paginated token usage history. Allows service_role (Edge Functions) and authenticated users accessing own data.';

-- ============================================================================
-- Fix 2: Update get_user_token_usage_stats() to allow service_role
-- ============================================================================
...
COMMENT ON FUNCTION public.get_user_token_usage_stats IS
  'Retrieves aggregate token usage statistics. Allows service_role (Edge Functions) and authenticated users accessing own data.';

-- ============================================================================
-- Grant EXECUTE to service_role
-- ============================================================================

-- Explicitly grant service_role permission to execute these functions
...
DO $$
BEGIN
  RAISE NOTICE '✅ Token usage functions updated to allow service_role access';
  RAISE NOTICE '✅ Edge Functions can now query usage history on behalf of authenticated users';
  RAISE NOTICE '🎉 Service role authorization fix completed successfully!';
END $$;
