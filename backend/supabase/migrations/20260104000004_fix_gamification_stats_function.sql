-- Migration: Fix get_user_gamification_stats function
-- Date: 2026-01-04
-- Description: Updates get_user_gamification_stats to work without user_study_streaks table
--              Returns zero values for streak data until proper streak tracking is implemented
...
-- Recreate function without user_study_streaks dependency
...
BEGIN
    RETURN QUERY
    WITH xp_data AS (
        -- Total XP from topic progress only (no achievement_xp from streaks)
        SELECT
    ...
    rank_data AS (
        -- Leaderboard rank based on topic XP
        SELECT r.rank
        FROM (
            SELECT
                up.id,
                ROW_NUMBER() OVER (
                    ORDER BY COALESCE(SUM(utp.xp_earned), 0) DESC
                )::BIGINT AS rank
    ...
    study_streak_data AS (
        -- Calculate study streak from user_topic_progress
        -- TODO: Implement proper streak tracking
        SELECT
            0 AS current_streak,
            0 AS longest_streak,
            MAX(utp.completed_at)::date AS last_study_date,
            COUNT(DISTINCT DATE(utp.completed_at)) AS total_study_days
        FROM user_topic_progress utp
        WHERE utp.user_id = p_user_id
    ...
    SELECT
        ...
        COALESCE(ssd.total_study_days::integer, 0) AS total_study_days,
...
Updated 2026-01-04: Removed dependency on user_study_streaks table.
Study streaks currently return 0 until proper streak tracking is implemented.
Daily verse streaks are still tracked via daily_verse_streaks table.';
