-- Revert identifier parameter back to TEXT to support both user IDs and session IDs
-- Migration: 20250913000003_revert_identifier_to_text.sql
-- Issue: Function was changed to UUID but needs to support both UUID (user_id) and TEXT (session_id)

-- Explicitly drop the UUID version to prevent function overloading conflict
DROP FUNCTION IF EXISTS get_or_create_user_tokens(UUID, TEXT);
